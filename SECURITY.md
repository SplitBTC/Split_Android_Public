...
Split Android is a Bitcoin wallet app. Please treat potential vulnerabilities with care and report them privately.
...
- device model, Android version, app build/version, and whether the device is rooted
...
Security-sensitive areas in the Android app include:

- wallet seed phrase creation, restore, display, deletion, and local storage
- encrypted local storage for wallet seeds, remote-node credentials, NWC secrets, messaging keys, and related local secrets
- app lock, biometric/PIN-gated flows, and sensitive UI states
...
- QR parsing, intent handling, and payment request parsing
...
- Firebase/FCM token handling and notification routing
- dependency vulnerabilities and supply-chain concerns
- configuration that could expose private infrastructure, signing material, Firebase configuration, or production secrets
...
- attacks requiring an already-rooted device unless there is an app-specific failure

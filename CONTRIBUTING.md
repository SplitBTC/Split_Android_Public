...
Thanks for your interest in contributing to Split Android.

Split Android is a Bitcoin wallet app. Changes can affect wallet custody, local secrets, wallet-authenticated sessions, messaging, payments, rewards, and released mobile clients. Please approach contributions with that context in mind.
...
- encrypted preferences, Android Keystore, app lock, and sensitive UI states
...
- QR parsing, intents, and payment request parsing
...
- manifest, exported components, network security config, package ID, and signing configuration
- dependency updates and Gradle lockfile/build changes
...
Public-safe build configuration lives in:
...
```text
split.local.properties.example
```
